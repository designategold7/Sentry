...
from sentry.sql import database, init_db
    ...
    def backfill_column(self, table, old_columns, new_columns, pkeys=None, cast_funcs=None):
        ...
        with database.transaction() as txn:
            for values in q:
                idx += 1
                if idx % 10000 == 0:
                    print('[{:.2f}s] Backfilling {} {}/{} (wrote {})'.format(time.time() - start, str(table), idx, total, modified))
                
                # Commit in chunks to prevent memory bloat
                if modified % 1000 == 0 and modified > 0:
                    txn.commit()
                    
                # FIXED: Corrected array index shift operator precedence
                obj = {
        ...
        txn.commit()
        print('DONE, {} scanned {} written'.format(idx, modified))

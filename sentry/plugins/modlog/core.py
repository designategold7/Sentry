...
class ModLogPlugin(Plugin):
    ...
    def resolve_channels(self, guild, config):
        ...
        for key, channel in config.channels.items():
            ...
            if not chan:
                raise MetaException('Failed to ModLog.resolve_channels', {
                    ...
                    'key': str(key),
                    'config_channels': list(str(i) for i in config.channels.keys()),
                    'guild_channels': list(str(i) for i in guild.channels.keys()),
                })
        ...
        config._custom = None
        if config.custom:
            sentry_guild = self.call('CorePlugin.get_guild', guild.id)

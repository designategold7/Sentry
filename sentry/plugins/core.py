...
BOT_INFO = '''
Sentry is a moderation and utilitarian bot built for large Discord servers.
'''
...
class CorePlugin(Plugin):
    def load(self, ctx):
        ...
        # AMPUTATED: gevent_inotifyx is a dead C-extension that breaks Python 3 builds.
        # if ENV != 'prod':
    ...
    # AMPUTATED due to missing/broken gevent_inotifyx dependency
    ...
    def update_sentry_guild_access(self):
        ...
        users_who_have_access = {
            ...
            if SENTRY_USER_ROLE_ID in i.roles
        }
    ...
    def _attach_local_event_data(self, event, plugin_name, guild_id):
        ...
        # Kept as rowboat_guild to prevent breaking downstream plugins that expect this exact proxy name
    ...
    def send_control_message(self):
        embed = MessageEmbed()
        embed.set_footer(text='Sentry {}'.format(
    ...
    def command_uptime(self, event):
        event.msg.reply('Sentry was started {}'.format(
            humanize.naturaldelta(datetime.now(timezone.utc) - self.startup)
        ))
    ...
    def command_eval(self, event):
        ...
        if src.count('\n'):
            ...
            try:
                # Python 3 Syntax Fix for exec/compile
    ...
    def guild_join(self, event, guild):
        ...
        # Python 3 Syntax Fix for dictionary iteration
